module.exports = class Restrouter {
  ...
  respond(method,query,res, optionalLimit){
	var m = this._class[method](query);
	if(this.populate){
	  m.populate(this.populate);
	}
	if(optionalLimit || this.limit){
	  m.limit(optionalLimit || this.limit);
	}
	if(this.sort){
	  m.sort(this.sort);
	}
  ...
  get(){
	...
	this.app.get(this.baseRoute + 'find/*',(req,res)=>{
	  ...
	  var limit = searchStr.split('/limit/');
	  if (limit.length > 1) {
	  	searchStr = limit[0];
		limit = limit[1] / 1;
	  } else {
	  ...
	  this.respond('find',searchObj,res, limit);
	});
